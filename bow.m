...
for ii = 1:nimages
      ...
      load(inFName, 'features');
      histogram1=zeros(1,500);
      
          d = EuclideanDistance(features.data, C);
          [minv,index] = min(d');
 
          histogram1(index)=histogram1(index)+1;
 
      BoW(ii,:) = do_normalize(histogram1);
...
idright = right_v(1);% set number you are about to visualize
